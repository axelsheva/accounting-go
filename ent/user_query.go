...
func (uq *UserQuery) QueryBalances() *BalanceQuery {
	query := (&BalanceClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.O2M, false, user.BalancesTable, user.BalancesColumn),
		)
		fromU = sqlgraph.SetNeighbors(uq.driver.Dialect(), step)
		return fromU, nil
	}
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withBalances:     uq.withBalances.Clone(),
...
func (uq *UserQuery) WithBalances(opts ...func(*BalanceQuery)) *UserQuery {
	query := (&BalanceClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withBalances = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withBalances; query != nil {
		if err := uq.loadBalances(ctx, query, nodes,
			func(n *User) { n.Edges.Balances = []*Balance{} },
			func(n *User, e *Balance) { n.Edges.Balances = append(n.Edges.Balances, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (uq *UserQuery) loadBalances(ctx context.Context, query *BalanceQuery, nodes []*User, init func(*User), assign func(*User, *Balance)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(balance.FieldUserID)
	}
	query.Where(predicate.Balance(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.BalancesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
